...
class HomeFragment: Fragment() {
    ...
    private fun setupObservers(homeFragmentBinding: FragmentHomeBinding) {
        homeViewModel.errorLiveData.observe(viewLifecycleOwner, {
            logger.e(
                LOG_TAG, "homeViewModel.errorLiveData::$it"
            )
            showErrorDialog(it)
        })
    ...
    private fun showErrorDialog(errorMessage: String) {
        val errorDialog = BottomDialogDismissibleErrorMessage.newInstance(
            title = getString(R.string.generic_string_ERROR), message = errorMessage
        )
        errorDialog.show(
            childFragmentManager, "openAboutDialog"
        )
    }
    ...
    private fun setupAddSessionPresetFab(homeFragmentBinding: FragmentHomeBinding) {
        homeFragmentBinding.addSessionPresetFabSpeedDial.setSpeedDialItems(
            buildHomeFabSpeedDials(), logger
        )
    }

    private fun buildHomeFabSpeedDials(): List<FabSpeedDialItem> {
        val createFreeTimedSession = FabSpeedDialItem(
            R.drawable.ic_timer, {openTimedFreeSessionPresetDialog()}, getString(R.string.free_timed_session)
        )
        //
        val createTimedSession = FabSpeedDialItem(
            R.drawable.ic_hourglass, {openTimedSessionPresetDialog()}, getString(R.string.timed_session)
        )
        //
        val createAudioSession = FabSpeedDialItem(
            R.drawable.ic_audio_file, {openAudioSessionPresetDialog()}, getString(R.string.audio_session)
        )
        //
        val createFreeAudioSession = FabSpeedDialItem(
            R.drawable.ic_audio_search, {openAudioFreeSessionPresetDialog()}, getString(R.string.free_audio_session)
        )
        //
        return listOf(
            createFreeTimedSession, createTimedSession, createFreeAudioSession, createAudioSession
        )
    }

    private fun openAudioFreeSessionPresetDialog(preset: SessionPreset.AudioFreeSessionPreset? = null) {
        logger.d(
            LOG_TAG, "openAudioFreeSessionPresetDialog"
        )
    ...
    private fun openEditSessionPresetDialog(preset: SessionPreset) {
        when (preset) {
            ...
            is SessionPreset.TimedFreeSessionPreset -> openTimedFreeSessionPresetDialog(preset)
                ...
                LOG_TAG, "openEditSessionPresetDialog::UNKNOWN SESSION PRESET TYPE => doing nothing"
            )
        }
    }
                    ...
                    when (newState) {
                        SCROLL_STATE_IDLE -> {
                            showHideAddSessionPresetButton(
                                homeFragmentBinding, true
                            )
                        }

                        SCROLL_STATE_DRAGGING -> {
                            showHideAddSessionPresetButton(
                                homeFragmentBinding, false
                            )
                        }
                    }
                }
    ...
    private fun getSwipeHandler(context: Context): SwipeInRecyclerViewCallback {
        val editIcon = ContextCompat.getDrawable(
            context, R.drawable.ic_button_edit
        )
        val moveToTopIcon = ContextCompat.getDrawable(
            context, R.drawable.ic_move_to_top
        )
        return object: SwipeInRecyclerViewCallback(
            rightIcon = editIcon, leftIcon = moveToTopIcon, backgroundColor = null, logger = logger
        ) {
            override fun onSwiped(
                ...
                if (swipedPreset == null) {
                    logger.e(
                        LOG_TAG, "onSwiped:: could not retrieve SessionPreset for position $position"
                    )
                }
                else {
                    when (direction) {
                        ...
                        ItemTouchHelper.RIGHT -> homeViewModel.moveSessionPresetToTop(
                            swipedPreset, resources.getString(R.string.no_session_preset_found)
                        )
                    }
                }
            }
    ...
    private fun showHideAddSessionPresetButton(
        ...
        if (showIt) {
            homeFragmentBinding.addSessionPresetFabSpeedDial.animateAlpha(
                fromAlpha = 0f, toAlpha = 1f, duration = Constants.STANDARD_ANIMATION_DURATION_MILLIS
            ).start()
        }
        else {
            homeFragmentBinding.addSessionPresetFabSpeedDial.animateAlpha(
                fromAlpha = 1f, toAlpha = 0f, duration = Constants.STANDARD_ANIMATION_DURATION_MILLIS
            ).start()
        }
    }
}