...
abstract class SessionDao{
    ...
    abstract fun getAllSessionsStartTimeDesc(): LiveData<List<SessionDTO>?>

    @Query("SELECT * from sessions_table ORDER BY realDuration ASC")
    abstract fun getAllSessionsDurationAsc(): LiveData<List<SessionDTO>?>

    @Query("SELECT * from sessions_table ORDER BY realDuration DESC")
    abstract fun getAllSessionsDurationDesc(): LiveData<List<SessionDTO>?>

    //Sessions WITH audio file guideMp3
    @Query("SELECT * from sessions_table WHERE guidemp3 != '' ORDER BY startTimeOfRecord DESC")
    abstract fun getAllSessionsWithMp3(): LiveData<List<SessionDTO>?>
